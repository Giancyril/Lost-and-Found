...
import {
  FaTrophy, FaMedal, FaClock, FaChartLine,
} from "react-icons/fa";
import {
                    ...
                    <div key={cat.name} className="space-y-1.5">
                      ...
                      <div className="w-full bg-gray-800 rounded-full h-1.5 overflow-hidden">
                        <div className="h-full flex rounded-full overflow-hidden" style={{ width: `${pct}%` }}>
                          {cat.found > 0 && (
                            <div className="bg-cyan-500 h-full" style={{ width: `${Math.round((cat.found / cat.total) * 100)}%` }} />
                          )}
                          {cat.lost > 0 && (
                            <div className="bg-red-500 h-full" style={{ width: `${Math.round((cat.lost / cat.total) * 100)}%` }} />
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-gray-900 border border-white/5 rounded-2xl p-5 space-y-4">
            ...
            <div className="text-center py-2">
              {stats?.avgClaimResolutionDays != null ? (
                <>
                  <p className="text-5xl font-bold text-violet-400 tracking-tight">
                    {stats.avgClaimResolutionDays}
                  </p>
                  <p className="text-gray-400 text-sm mt-1.5 font-medium">days on average</p>
                  <p className="text-gray-600 text-[11px] mt-3">
                    Based on {(stats?.approvedClaims ?? 0) + (stats?.rejectedClaims ?? 0)} resolved claims
                  </p>
                </>
              ) : (
                <div className="py-4">
            ...
            <div className="divide-y divide-white/5">
              {topReporters.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-gray-600">
                  ...
                  <p className="text-xs mt-1 opacity-60">Data appears once items are reported</p>
                </div>

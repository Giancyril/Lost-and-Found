...
import {
  ...
  FaCheckCircle, FaTimesCircle, FaBan, FaUser, FaComment,
  FaShieldAlt, FaSpinner, FaFilter, FaSearch, FaBolt,
} from "react-icons/fa";
...
// ── Confirm modal ─────────────────────────────────────────────────────────────
const ConfirmModal = ({ title, message, onConfirm, onCancel, confirmLabel = "Confirm", danger = false }: any) => (
  <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl p-5 space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={onCancel} className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 border border-white/5 text-gray-300 rounded-xl text-sm font-medium transition-colors">Cancel</button>
        <button onClick={onConfirm} className={`flex-1 py-2.5 text-white rounded-xl text-sm font-semibold transition-colors ${danger ? "bg-red-600 hover:bg-red-500" : "bg-cyan-600 hover:bg-cyan-500"}`}>
          {confirmLabel}
        </button>
      </div>
    </div>
...
const ReportedContentTab = () => {
  ...
  const handleResolve = async (id: string, action: string, commentAction: string) => {
    ...
    if (res?.data?.success) { toast.success("Report resolved"); setSelectedReport(null); }
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard
        title="User Reports"
        subtitle="Comments flagged by users as inappropriate"
        action={
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0 space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                          isPending ? "bg-yellow-400/10 border-yellow-400/20 text-yellow-400" :
                          r.status === "REVIEWED" ? "bg-emerald-400/10 border-emerald-400/20 text-emerald-400" :
                      ...
                      {r.comment && (
                        <div className="p-2.5 bg-gray-800/60 border border-white/5 rounded-lg">
                          <p className="text-gray-400 text-[10px] mb-1">
                            Comment by <span className="text-white font-semibold">{r.comment.user?.username || "Anonymous"}</span>
                            {" · "}{r.comment.itemType} item
                          </p>
                    ...
                    {isPending && (
                      <div className="flex flex-col gap-1.5 shrink-0">
                        <button onClick={() => handleResolve(r.id, "REVIEWED", "REJECT")}
                          ...
                          <FaBan size={8} /> Reject Comment
                        </button>
                        <button onClick={() => handleResolve(r.id, "REVIEWED", "APPROVE")}
                          ...
                          <FaCheck size={8} /> Keep Comment
                        </button>
                        <button onClick={() => handleResolve(r.id, "DISMISSED", "NONE")}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-500/10 hover:bg-gray-500/20 border border-gray-500/20 text-gray-400 text-[10px] font-bold rounded-lg transition-all">
                          <FaTimes size={8} /> Dismiss
                        </button>
                      </div>
                    )}
                    <button onClick={() => deleteReport(r.id)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-red-500/10 border border-white/5 text-gray-600 hover:text-red-400 flex items-center justify-center transition-all shrink-0">
                      <FaTrash size={8} />
                    </button>
                  </div>
                </div>
      ...
      <SectionCard
        ...
        subtitle="Review comments flagged by auto-moderation"
        action={
          <div className="flex gap-1 bg-gray-800 border border-white/5 rounded-lg p-1">
              ...
              <div key={c.id} className="px-5 py-4 hover:bg-white/[0.02] transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0 space-y-1.5">
                    ...
                    {c.rejectionReason && (
                      <p className="text-red-400/70 text-[10px]">Rejection reason: {c.rejectionReason}</p>
                    )}
                    <p className="text-gray-700 text-[10px]">{timeAgo(c.createdAt)}</p>
                  </div>
                  <div className="flex flex-col gap-1.5 shrink-0">
                    <button onClick={() => handleModerate(c.id, "APPROVED")}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold rounded-lg transition-all">
                      <FaCheck size={8} /> Approve
                    </button>
                    <button onClick={() => handleModerate(c.id, "REJECTED")}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 text-[10px] font-bold rounded-lg transition-all">
...
const UserBehaviorTab = () => {
  ...
  const handleIssueWarning = async () => {
    ...
    if (res?.data?.success) {
      ...
      setWarnForm({ reason: "", severity: "LOW", note: "" });
      refetch();
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="Users with Warnings" subtitle="Users who have received moderation warnings">
        {(behavior?.userBehaviorList || []).length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-gray-600">
              ...
              <div key={item.user?.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {item.highCount > 0 && (
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-red-400/10 border border-red-400/20 text-red-400">
                        {item.highCount} HIGH
                      </span>
                    )}
                    {!item.user?.activated && (
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-yellow-400/10 border border-yellow-400/20 text-yellow-400">Blocked</span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right">
                    <p className="text-orange-400 text-sm font-bold">{item.count}</p>
                    <p className="text-gray-600 text-[10px]">warnings</p>
                  </div>
                  <div className="flex gap-1.5">
                    <button onClick={() => { setWarnTarget(item.user); setShowWarnModal(true); }}
                      className="w-7 h-7 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20 text-orange-400 flex items-center justify-center transition-all" title="Issue warning">
                      <FaExclamationTriangle size={9} />
                    </button>
                    {item.user?.activated && (
                      <button onClick={() => handleBlock(item.user.id, item.user.username)}
                        className="w-7 h-7 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 flex items-center justify-center transition-all" title="Block user">
                        <FaBan size={9} />
                      </button>
                    )}
                  </div>
      ...
      <SectionCard title="Recent Warnings" subtitle="Latest warnings issued by admins">
        {(behavior?.recentWarnings || []).length === 0 ? (
            ...
            {(behavior?.recentWarnings || []).map((w: any) => {
              ...
              return (
                <div key={w.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/[0.02] transition-colors">
                  <div className={`w-7 h-7 rounded-xl border flex items-center justify-center shrink-0 ${sc.bg}`}>
                    <FaExclamationTriangle size={9} className={sc.color} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => deleteWarning(w.id)}
                    className="w-7 h-7 rounded-lg bg-white/5 hover:bg-red-500/10 border border-white/5 text-gray-600 hover:text-red-400 flex items-center justify-center transition-all shrink-0">
                    <FaTrash size={8} />
                  </button>
                </div>
              );
      ...
      {showWarnModal && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl" style={{ borderTop: "2px solid #f97316" }}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
              <div>
                <h3 className="text-white text-sm font-bold">Issue Warning</h3>
                <p className="text-gray-500 text-[11px] mt-0.5">Warning for {warnTarget?.username}</p>
              </div>
              <button onClick={() => setShowWarnModal(false)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-colors">
                <FaTimes size={12} />
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input value={warnForm.reason} onChange={e => setWarnForm(f => ({ ...f, reason: e.target.value }))}
                  placeholder="e.g. Repeated spam comments"
              ...
              <div className="space-y-1.5">
                ...
                <textarea value={warnForm.note} onChange={e => setWarnForm(f => ({ ...f, note: e.target.value }))}
            ...
            <div className="px-5 py-4 border-t border-white/5 flex gap-3">
              <button onClick={() => setShowWarnModal(false)} className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 border border-white/5 text-gray-300 rounded-xl text-sm font-medium transition-colors">Cancel</button>
              <button onClick={handleIssueWarning}
                className="flex-1 py-2.5 bg-orange-600 hover:bg-orange-500 text-white rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
      ...
      <div className="bg-gray-900 border border-white/5 rounded-2xl p-5 space-y-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center">
            <FaRobot size={16} className="text-cyan-400" />
          </div>
          <div>
            <h3 className="text-white text-sm font-bold">How Auto-Moderation Works</h3>
            <p className="text-gray-500 text-[10px]">No AI API cost — runs locally on every comment submission</p>
          </div>
        </div>
        <div className="space-y-2.5">
          {[
            ...
            { step: "3", title: "Auto-decision",      desc: "Clean content → APPROVED immediately. Flagged content → PENDING for admin review" },
        ...
        <div className="p-5 space-y-3">
          <div className="flex flex-wrap gap-2">
            {keywords.map((kw: string) => (
              <span key={kw} className="px-2.5 py-1 bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-mono rounded-lg">
                {kw}
              </span>
            ))}
          </div>
          <div className="pt-2 border-t border-white/5">
            <p className="text-gray-600 text-[10px] leading-relaxed">
              To add or remove keywords, edit the <code className="bg-gray-800 px-1 rounded text-cyan-300 text-[10px]">BLOCKED_KEYWORDS</code> array in{" "}
      ...
      <SectionCard title="Content Tester" subtitle="Test a piece of text against the keyword filter">
        <div className="p-5 space-y-4">
          <textarea value={testText} onChange={e => { setTestText(e.target.value); setTestResult(null); }}
            rows={4} placeholder="Paste or type any text here to test it against the filter..."
          ...
          <button onClick={handleTest} disabled={isTesting}
            className="flex items-center gap-2 px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all">
            {isTesting ? <><FaSpinner className="animate-spin" size={11} /> Testing…</> : <><FaBolt size={11} /> Test Content</>}
          </button>
          {testResult && (
            <div className={`p-4 rounded-xl border flex items-start gap-3 ${testResult.clean ? "bg-emerald-500/5 border-emerald-500/20" : "bg-red-500/5 border-red-500/20"}`}>
              {testResult.clean
                ? <FaCheckCircle size={16} className="text-emerald-400 shrink-0 mt-0.5" />
                : <FaTimesCircle size={16} className="text-red-400 shrink-0 mt-0.5" />}
              <div>
                ...
                <p className="text-gray-500 text-xs mt-0.5">
                  {testResult.clean
                    ? "This text would be auto-approved when submitted as a comment."
...
const AppealProcessTab = () => {
  ...
  const handleResolve = async (action: "APPROVED" | "DENIED") => {
    ...
    if (res?.data?.success) {
      toast.success(action === "APPROVED" ? "Appeal approved — comment restored" : "Appeal denied");
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard
        ...
        subtitle="Users disputing rejected comments"
        action={
          <div className="flex gap-1 bg-gray-800 border border-white/5 rounded-lg p-1">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0 space-y-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                          isPending ? "bg-violet-400/10 border-violet-400/20 text-violet-400" :
                          a.status === "APPROVED" ? "bg-emerald-400/10 border-emerald-400/20 text-emerald-400" :
                      ...
                      {a.comment && (
                        <div className="p-2.5 bg-gray-800/60 border border-white/5 rounded-lg">
                          ...
                          {a.comment.rejectionReason && (
                            <p className="text-red-400/60 text-[10px] mt-1">Reason: {a.comment.rejectionReason}</p>
                          )}
                        </div>
                      )}
                      ...
                      {a.adminNote && (
                        <p className="text-gray-600 text-[10px] italic">Admin note: {a.adminNote}</p>
                      )}
      ...
      {selectedAppeal && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl" style={{ borderTop: "2px solid #a78bfa" }}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
              <div>
                <h3 className="text-white text-sm font-bold">Review Appeal</h3>
                <p className="text-gray-500 text-[11px] mt-0.5">From {selectedAppeal.user?.username || "Anonymous"}</p>
              </div>
              <button onClick={() => setSelectedAppeal(null)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-colors">
                <FaTimes size={12} />
              </button>
            </div>
            <div className="p-5 space-y-4">
              {selectedAppeal.comment && (
                <div className="p-3 bg-gray-800/60 border border-white/5 rounded-xl space-y-1">
                  ...
                  {selectedAppeal.comment.rejectionReason && (
                    <p className="text-red-400/60 text-[10px]">Rejection reason: {selectedAppeal.comment.rejectionReason}</p>
                  )}
                </div>
              )}
            ...
            <div className="px-5 py-4 border-t border-white/5 flex gap-3">
              <button onClick={() => handleResolve("DENIED")}
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
                <FaTimes size={10} /> Deny Appeal
              </button>
              <button onClick={() => handleResolve("APPROVED")}
                ...
                <FaCheck size={10} /> Approve & Restore
              </button>
            </div>
          </div>
        </div>

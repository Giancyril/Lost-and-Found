...
import {
  ...
  FaFilter, FaSpinner, FaStar, FaRegStar, FaTag,
} from "react-icons/fa";
...
// ── RTK Query endpoints (injected inline) ─────────────────────────────────────
        ...
        <Modal onClose={() => setShowCompose(false)} wide>
          ...
          <div className="p-5 overflow-y-auto space-y-4" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.2) rgba(255,255,255,0.05)" }}>
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Title <span className="text-red-400">*</span></label>
                  ...
                  <div className="flex flex-col gap-1.5 shrink-0">
                    ...
                    {t.status === "OPEN" && (
                      <button onClick={() => handleStatusChange(t.id, "IN_PROGRESS")}
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/20 text-yellow-400 text-[10px] font-bold rounded-lg transition-all">
                        <FaEye size={8} /> View
                      </button>
                    )}
                    <button onClick={() => handleDelete(t.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-red-500/10 border border-white/5 text-gray-600 hover:text-red-400 text-[10px] font-bold rounded-lg transition-all">
      ...
      {selectedTicket && (
        <Modal onClose={() => setSelectedTicket(null)} wide>
          ...
          <div className="p-5 overflow-y-auto space-y-4" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.2) rgba(255,255,255,0.05)" }}>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Your Reply <span className="text-red-400">*</span></label>
              <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                rows={5} placeholder="Write your response..."
                className="w-full px-4 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm placeholder-gray-600 resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500/30" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Set Status After Reply</label>
    ...
    <div className="space-y-4 max-w-2xl">
      <div className="bg-gray-900 border border-white/5 rounded-2xl overflow-hidden">
        ...
        {sent && lastResult ? (
          ...
          <div className="p-5 space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Subject / Title <span className="text-red-400">*</span></label>
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Message <span className="text-red-400">*</span></label>
              <textarea value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                rows={6} placeholder="Write your notification message..."
                className="w-full px-4 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm placeholder-gray-600 resize-none focus:outline-none focus:ring-2 focus:ring-cyan-500/30" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Recipients</label>
                <select value={form.target} onChange={e => setForm(f => ({ ...f, target: e.target.value }))}
                  className="w-full px-3 py-2.5 bg-gray-800 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/30">
                  <option value="ALL">All Users</option>
                  <option value="STUDENTS">Students Only</option>
            ...
            <div className="p-3 bg-yellow-500/5 border border-yellow-500/15 rounded-xl flex items-start gap-2">
              <FaExclamationTriangle size={10} className="text-yellow-400 shrink-0 mt-0.5" />
              <p className="text-yellow-300/70 text-xs leading-relaxed">This will send an email to <strong className="text-yellow-300">{form.target === "ALL" ? "all active users" : form.target.toLowerCase()}</strong>. Make sure the message is accurate before broadcasting.</p>
            </div>
            <button onClick={handleBroadcast} disabled={isBroadcasting}
              className="w-full py-3 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard label="Open Tickets" value={stats?.openTickets} color="text-yellow-400" bg="bg-yellow-400/10 border-yellow-400/20"
          icon={<FaTicketAlt size={14} className="text-yellow-400" />} />
        <StatCard label="Urgent Tickets" value={stats?.urgentTickets} color="text-red-400" bg="bg-red-400/10 border-red-400/20"
          icon={<FaFire size={14} className="text-red-400" />} />
        <StatCard label="Unread Feedback" value={stats?.unresolvedFeedback} color="text-violet-400" bg="bg-violet-400/10 border-violet-400/20"
          icon={<FaCommentDots size={14} className="text-violet-400" />} />
        <StatCard label="Announcements" value={stats?.totalAnnouncements} color="text-cyan-400" bg="bg-cyan-400/10 border-cyan-400/20"
          icon={<FaBullhorn size={14} className="text-cyan-400" />} />
      </div>

      {/* Tabs */}
      <div className="grid grid-cols-2 sm:flex bg-gray-900 border border-white/5 rounded-2xl p-1 gap-1">
        {TABS.map(tab => {
            const Icon = tab.icon;
            const active = activeTab === tab.id;
            return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors sm:flex-1 focus:outline-none select-none

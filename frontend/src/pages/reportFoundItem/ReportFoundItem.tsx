...
const ReportFoundItem = () => {
  ...
  // Live watched values — reliable even for Controller-managed fields
  ...
  const handleFileChange = async (files: FileList | null) => {
    ...
    const compressedFiles = await Promise.all(valid.map(async (file) => {
      ...
      catch (error) { console.error("Image compression error:", error); return file; }
    }));
  ...
  const handleScan = (student: ScannedStudent) => {
    ...
    // ScannedStudent only has `department` — no `course` field on this type
  ...
  const handleFetchDetails = async () => {
    ...
    try {
      ...
      if (name) {
        ...
        } catch { /* fall through to email-only */ }
      }
      ...
      if (student?.name) {
        // `student` here is a raw API response object — it has both `department` and `course`
        const dept = student.department || student.course || "";
        setValue("reporterName", student.name);
        setValue("schoolEmail",  student.email);
        setValue("department",   dept);
        // Build a ScannedStudent — only use fields that exist on ScannedStudent type
        setScannedStudent({
          id:         student.id   || "",
          name:       student.name || "",
          email:      student.email || "",
      ...
      <section className="min-h-screen flex items-center justify-center bg-gray-950 py-4 px-3">
        <div className="w-full max-w-xs mx-auto">
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6 sm:p-10 text-center">
            ...
            <p className="text-gray-600 text-xs mt-8">Thank you for being honest and helping return lost items to their owners.</p>
          </div>
        </div>
      </section>
    );
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4 sm:p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
              <div className="flex items-center gap-4 text-left">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-1">Log a discovered item into the system.</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid gap-6 md:grid-cols-2">
                ...
                {/* Name + Email */}
                <div className="md:col-span-2 grid gap-4 sm:gap-5 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <div className={`relative flex items-center ${inputCls} ring-0 focus-within:ring-2 focus-within:ring-emerald-500/50`}>
                      ...
                      <input {...register("reporterName", { required: "Finder's name is required" })}
                        type="text" className="bg-transparent border-none p-0 w-full focus:ring-0 text-sm" placeholder="Enter name or scan ID" />
                    </div>
                    {errors.reporterName && <p className="text-red-400 text-xs mt-1 text-left">{errors.reporterName?.message as string}</p>}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <div className={`relative flex items-center ${inputCls} ring-0 focus-within:ring-2 focus-within:ring-emerald-500/50`}>
                      ...
                      <input {...register("schoolEmail", {
                        required: "School email is required",
                        pattern: { value: /^[^\s@]+@nbsc\.edu\.ph$/i, message: "Must be a valid NBSC email" },
                      })} type="email" autoComplete="off" className="bg-transparent border-none p-0 w-full focus:ring-0 text-sm" placeholder=" " />
                    </div>
                ...
                <div className="md:col-span-2 space-y-1.5">
                  ...
                  <div className={`relative flex items-center ${inputCls} bg-gray-800/40 opacity-80 ring-0`}>
                    ...
                    <input {...register("department")} type="text" readOnly
                      className="bg-transparent border-none p-0 w-full focus:ring-0 text-sm italic" placeholder="Auto-filled from masterlist..." />
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <button type="button" onClick={() => setShowCategoryHelp(true)}
                      className="w-4 h-4 rounded-full bg-gray-700 hover:bg-gray-600 border border-gray-600 text-gray-400 hover:text-white flex items-center justify-center transition-all" title="About categories">
              ...
              <div>
                <label className="block mb-1.5 text-xs font-bold text-white uppercase tracking-widest">
                  Item Photos <span className="text-gray-500 normal-case font-normal">(up to {MAX_IMAGES} images)</span>
                </label>
                <div
                  className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all duration-200 ${isDragging ? "border-blue-500 bg-blue-900/10" : "border-gray-700 bg-gray-800/50 hover:border-blue-500 hover:bg-gray-800"}`}
                  ...
                  <div className="text-gray-400 text-sm">
                    ...
                    <p className="text-xs text-gray-600 mt-1">JPG, PNG, WEBP · Max {MAX_SIZE_MB}MB each · {selectedFiles.length}/{MAX_IMAGES} selected</p>
                  </div>
                </div>
                  ...
                  <>
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mt-3">
                      {previews.map((src, idx) => (
                        <div key={idx}
                          className={`relative rounded-lg overflow-hidden aspect-square border-2 transition-all duration-200 cursor-pointer ${idx === primaryIdx ? "border-blue-500 ring-2 ring-blue-500/30" : "border-gray-700 hover:border-gray-500"}`}
                          onClick={() => setPrimaryIdx(idx)} title="Click to set as cover photo">
                          <img src={src} className="w-full h-full object-cover" alt="" />
                          {idx === primaryIdx && (
                            <div className="absolute bottom-0 left-0 right-0 bg-blue-600/90 text-white text-center text-[10px] font-bold py-0.5">Cover</div>
                          )}
                          <button type="button" onClick={(e) => { e.stopPropagation(); removeFile(idx); }}
                            className="absolute top-1 right-1 w-5 h-5 bg-black/60 hover:bg-red-600 rounded-full text-white text-xs flex items-center justify-center transition-colors duration-150">✕</button>
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-sm shadow-2xl flex flex-col">
            ...
            <div className="px-5 py-5 flex-1 flex flex-col justify-between min-h-[260px]">
              <div className="space-y-4">
                <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest flex items-center gap-1.5">{CATEGORY_HELP_CONTENT.tag}</p>

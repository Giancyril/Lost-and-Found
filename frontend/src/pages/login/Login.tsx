        ...
        <div className="relative z-10 w-full max-w-sm px-6 -mt-16">
          <div className="bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl shadow-black/50 p-6 transition-all duration-300 hover:shadow-3xl hover:shadow-blue-500/10">
            ...
            <div className="mb-6 text-center">
              <div className="flex flex-col items-center gap-2 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-medium text-blue-300/70 tracking-widest uppercase">Student Affairs and Services</span>
                  <span className="w-6 h-px bg-white/15" />
                </div>
                <h1 className="text-xl font-bold text-white tracking-tight">Lost & <span className="text-blue-400">Found</span></h1>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {/* Username */}
              <div>
                <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-300 uppercase tracking-wide mb-1.5">
                  <MdEmail className="w-3.5 h-3.5 text-blue-400" />
                  Email or Username
                </label>
                <div className="relative">
                  ...
                  {isFocused === 'username' && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      <MdEmail className="w-4 h-4 text-blue-400 opacity-60" />
                    </div>
                  )}
                </div>
                {errors.username && (
                  <div className="flex items-center gap-1.5 mt-1 text-red-400 text-xs animate-pulse">
              ...
              <div>
                <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-300 uppercase tracking-wide mb-1.5">
                  <MdLock className="w-3.5 h-3.5 text-blue-400" />
                  Password
                </label>
                <div className="relative">
                  <input
                  ...
                  <button
                    ...
                    className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-blue-400 transition-colors duration-300"
                  >
                    {showPassword
              ...
              {isLocked ? (
                <div className="w-full bg-red-500/20 border border-red-500/30 rounded-lg py-3 px-3 text-center space-y-2">
                  <div className="flex items-center justify-center gap-2">
                    <MdErrorOutline className="w-5 h-5 text-red-400" />
                    <div>
                      <p className="text-red-400 font-semibold text-sm">Account temporarily locked</p>
                      <p className="text-gray-400 text-xs">Too many failed login attempts</p>
                    </div>
                  </div>
                  <div className="bg-gray-800/60 rounded-md py-1.5 px-3">
                    <p className="text-gray-300 text-xs">
                      Try again in{" "}
                      <span className="text-white font-bold text-base">{formatCountdown(countdown)}</span>
                    </p>
                  </div>
                </div>
              ) : isLoading ? (
                <div className="flex justify-center py-2">
                  <div className="flex items-center gap-2 bg-blue-500/20 rounded-lg px-5 py-2.5">
                    <Spinner size="sm" />
                    <span className="text-blue-300 text-sm font-medium">Authenticating...</span>
                  </div>
                </div>
              ) : (
                <>
                  <button
                    type="submit"
                    className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-semibold py-2.5 px-4 rounded-lg transition-all duration-300 text-sm shadow-lg shadow-blue-900/30 hover:shadow-xl hover:shadow-blue-500/40 transform hover:scale-[1.02] active:scale-[0.98]"
                  >
                    Sign In
                  </button>
                  {attempts > 0 && (
                    <div className="mt-2 text-center">
                      <p className="text-xs text-orange-400 bg-orange-500/10 rounded-md py-1.5 px-3 inline-block">
